...
/**
 * return the correct path for DiscoverService or User's AMFService
 */
function getBasePath($class){
	if(preg_match('/DiscoveryService(\.php)*$/i', $class)){
		return $GLOBALS['amfphp']['classPath'];
	}
	return $GLOBALS['amfphp']['servicePath'];
}
...
function adapterAction (&$amfbody) {
	...
	$baseServicePath = $GLOBALS['amfphp']['servicePath'];
	...
	if (strpos($target, "http://") === false && strpos($target, "https://") === false) { // check for a http link which means web service
		...
		if ($lpos === false) {
			...
			if($messageType == 'flex.messaging.messages.RemotingMessage')
			{
				...
				$classpath = getBasePath($classAndPackage) . $uriclasspath;
			}
			...
			if ($lpos === false) {
				...
				if ($classname == "PageAbleResult" && $methodname == 'getRecords') {
					...
					$classpath = getBasePath($uriclasspath) . $set['class'];
				...
				else if($classname == "PageAbleResult" && $methodname == 'release')
				{
					...
					$classpath = getBasePath($uriclasspath) . $trunced . ".php";
				} 
			} else {
				$classname = substr($trunced, $lpos + 1);
				$classpath = getBasePath($classname) . str_replace(".", "/", $trunced) . ".php"; // removed to strip the basecp out of the equation here
