...
class Smarty_Compiler extends Smarty {
    ...
    function Smarty_Compiler()
    {
		...
		$this->_var_bracket_regexp = '\[\$?[\'"%#!*@\~\w\.]+\]';//iwind
    ...
    function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        //iwind code begin here
        if (!$found) {
        	$if_impl = "if_smarty_block_{$tag_command}";
        	if (function_exists($if_impl)) {
        		$found = true;
        		$have_function = true;
        		//add to plugin dynamicly
        		$this->register_block($tag_command, $if_impl);
        	}
        }
        //iwind code end here
    ...
    function _compile_custom_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
		...
		//iwind code begin here
        if (!$found) {
        	$if_impl = "if_smarty_func_{$tag_command}";
        	if (function_exists($if_impl)) {
        		$found = true;
        		$have_function = true;
        		//add to plugin dynamicly
        		$this->register_function($tag_command, $if_impl);
        	}
        }
        //iwind code end here
    ...
    function _compile_if_tag($tag_args, $elseif = false)
    {
        ...
        preg_match_all('~(?>' . $this->_obj_call_regexp . '(?:' . $this->_mod_regexp . '*)? |' . $this->_var_regexp . '(?:' . $this->_mod_regexp . '*)?    |\-?0[xX][0-9a-fA-F]+|\-?\d+(?:\.\d+)?|\.\d+|!==|===|==|!=|<>|<<|>>|<=|>=|\&\&|\|\||\(|\)|,|\!|\^|=|\&|\~|<|>|\||\%|\+|\-|\/|\*|\@|\b\w+\b|\S+)~x', $tag_args, $match);
            ...
            switch ($state) {
                case 0:
                    /* If the token is a valid identifier, we set attribute name
                       and go to state 1. */
                    if (preg_match('~^[\w\.@\*-:]+$~', $token)) {//iwind add dot (.) and -,@,* (v1.1.0),:(v1.1.1) to the pattern
                        $attr_name = $token;
                        $state = 1;
                    } else
                ...
                case 2:
                    ...
                    if ($token != '=') {
                        /* We booleanize the token if it's a non-quoted possible
                           boolean value. */
                        if (preg_match('~^(true)$~', $token)) {//iwind: on|yes|true => true
                            $token = 'true';
                        } else if (preg_match('~^(false)$~', $token)) {//iwind: off|no|false => false
                            $token = 'false';
                        } else if ($token == 'null') {
                            $token = 'null';
                ...
                if (substr($_index, 0, 1) == '[') {
                    ...
                    if (is_numeric($_index)) {
                        ...
                        if (strpos($_index, '.') !== false) {
						 ...
						 //iwind's code begin here
                    	$_index = trim($_index, '"\'');
                    	if ($_index && in_array(substr($_index, 0, 1), array("%", "#", "*", "!", "@", "~"))) {
                    		$_output .= "['" . $_index . "']";   
                    		continue;                 		
                    	}
                    	//iwind's code end here
